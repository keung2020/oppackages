...
if [ ! -f "/tmp/adbyby.updated" ];then
  ...
  while [ "$wget_ok" = "0" ] 
    ...
    if [ "$?" == "0" ]; then
      ...
      touch /tmp/md5.json && wget-ssl --no-check-certificate -t 1 -T 10 -O /tmp/md5.json https://github.com/keung2020/xwhyc-rules/blob/master/md5.json
      ...
      if [ "$adm5" == "$bmd5" ];then
        ...
        touch /tmp/lazy.txt && wget-ssl --no-check-certificate -t 1 -T 10 -O /tmp/lazy.txt https://github.com/keung2020/xwhyc-rules/blob/master/lazy.txt
        touch /tmp/video.txt && wget-ssl --no-check-certificate -t 1 -T 10 -O /tmp/video.txt https://github.com/keung2020/xwhyc-rules/blob/master/video.txt
